...
from AI_handler import AI_handler_with_input as gemini_prompts
...
class Main:
    def __init__(self, authority = True):
        self.startup_flag = False
        self.startup_loop_counter = -1
        self.error_counts = get_final_error_count()

        #make this false to send it to AI
        self.design_valid = authority
        if(self.design_valid):
            self.startup_flag = False
        elif((self.design_valid) == False):
    ...
    def startup(self):
        ...
        #====uncomment this to run the debugger===
        # self.startup_flag = True
        self.startup_loop_counter += 1
        #======================================
    ...
    def parse_design(self):
        ...
        sv_text = gemini_prompts()  # <-- must return SV code as a string
        gen_design_path = path_to_testbench()
        out_path = design_writer(gen_design_path, sv_text, mode="atomic")

        print(f"[OK] Wrote design to: {out_path}")
        print("holding delay for 5 seconds to confirm write")
        time.sleep(5)
...
if __name__ == "__main__":
    print("enter F to start\n")
    key = input("Enter your key: T/F : ").upper()
    key_control(key)

    app = Main(authority=key_control(key))
    app.startup()
